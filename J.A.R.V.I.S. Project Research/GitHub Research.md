...
This is also how the J.A.R.V.I.S. Project gathers all the necessary items that are required to make the correct API calls and gather the correct information so that way the rest of the GitHub API plugin that I built can do its job and separate the data into useable data and information that will end up being displayed to the end user. The ways that this is going to be done is by having a central settings menu that will have all of the information in separate tabs to keep everything neatly organized and then from there whatever command is sent through that menu would be translated by the SettingsHandler.py program that would then eventually change the true Settings JSON Document. That would in the end make modifying the settings and managing them easier for the end user as well. I also plan to make it easier for the end user to edit the .env file that store all of the API Keys and API Tokens that this plugin uses. This in the end users case would truly make things easier for them when trying to use the J.A.R.V.I.S. Project for any of their practical uses and not having them have to go into the actual program and edit these files also reduces the headache that would be trying to sieve through the files and then through the documentation to figure out what values need to be changed to get the proper affect or for them to reach the proper goal of what they are looking to achieve.